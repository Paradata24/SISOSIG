...
export async function GET(request: Request) {
  ...
  // Winddaten (Richtung + Geschwindigkeit) verwenden.
  let stationCode = requestedStation ?? undefined;
  if (!stationCode) {
    for (const [code, readings] of byStation) {
      if (readings.some(hasDirection) && readings.some(hasSpeed)) {
        stationCode = code;
        break;
      }
    }
  ...
  if (!stationCode || !byStation.has(stationCode)) {
    return NextResponse.json(
      { error: "Keine Station mit Winddaten gefunden" },
      { status: 404 },
    );
  }
